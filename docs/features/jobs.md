...
2. Change `create_note` to begin a transaction, insert the note and job intent on that transaction, then commit. No email/network call occurs inside it. On conflict of a dedupe key, confirm that the existing intent matches the requested operation. Return the note after commit; HTTP success means accepted intent, not completed delivery.
